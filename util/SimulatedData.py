...
fix_keypoint_indices = [395, 550, 756, 436, 952, 1069]
...
'''
to get edge connection
'''
mapdict = {}
for i in range(len(keypoint_indices)):
    ind = keypoint_indices[i]
    mapdict[ind] = i
def MeshEdgeMap(oldpoint_edgelist):
    newpoint_edgelist = []
    for edge in oldpoint_edgelist:
        newpoint_edgelist.append((mapdict[edge[0]], mapdict[edge[1]]))
    return newpoint_edgelist

def Edge2Mesh(ori_keypoint_edges):
    facelist = []
    for i in ori_keypoint_edges:
        for j in ori_keypoint_edges:
            pass
    return None

def Readface(topofile):
    face_array = np.genfromtxt(topofile,delimiter=',', dtype=int)
    return face_array

def TriFaceEdge():
    numFace = keypoint_mesh_face.shape[0]
    edgelist = []
    for i in range(numFace):
        e1 = [keypoint_mesh_face[i][0], keypoint_mesh_face[i][1]]
        e1_v = [keypoint_mesh_face[i][1], keypoint_mesh_face[i][0]]
        e2 = [keypoint_mesh_face[i][0], keypoint_mesh_face[i][2]]
        e2_v = [keypoint_mesh_face[i][2], keypoint_mesh_face[i][0]]
        e3 = [keypoint_mesh_face[i][1], keypoint_mesh_face[i][2]]
        e3_v = [keypoint_mesh_face[i][2], keypoint_mesh_face[i][1]]

        if (e1 in edgelist or e1_v in edgelist) is False:
            edgelist.append(e1)
        if (e2 in edgelist or e2_v in edgelist) is False:
            edgelist.append(e2)
        if (e3 in edgelist or e3_v in edgelist) is False:
            edgelist.append(e3)

    edgelist = np.array(edgelist)
    return edgelist

keypoint_edges_31 = MeshEdgeMap(keypoint_edges)
...
def RandomScale(inputPos, sx, sy, sz):
    inputPos[:,0] = inputPos[:,0] * sx
    inputPos[:, 1] = inputPos[:, 1] * sy
    inputPos[:,2] = inputPos[:,2] * sz
    return inputPos
